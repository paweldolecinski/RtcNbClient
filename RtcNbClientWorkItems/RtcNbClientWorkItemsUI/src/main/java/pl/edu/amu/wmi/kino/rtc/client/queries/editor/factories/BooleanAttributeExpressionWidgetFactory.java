...
public class BooleanAttributeExpressionWidgetFactory extends AbstractAttributeExpressionWidgetFactory {

    @Override
    public boolean canCreate(RtcEditableAttributeExpression expression,RtcQuery query) {
        return expression.getQueryAttribute().getLookup().lookup(RtcQueryAttributeBooleanValueProvider.class) != null;
    }

    @Override
    public Widget createDescription(RtcEditableAttributeExpression expression,RtcQuery query, Scene scene) {
        //TODO : bikol : implement boolean editor
        LayerWidget layer = new LayerWidget(scene);
        layer.addChild(new LabelWidget(scene, "Boolean"));
        return layer;
    }
