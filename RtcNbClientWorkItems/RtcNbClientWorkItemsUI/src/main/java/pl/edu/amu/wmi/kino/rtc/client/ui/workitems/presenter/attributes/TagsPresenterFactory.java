...
import pl.edu.amu.wmi.kino.rtc.client.facade.api.workitems.attributes.TagsMarker;
...
public class TagsPresenterFactory implements
    ...
    public double canCreate(WorkItemLayoutField field) {
        if (field.getType().equals(WorkItemLayoutField.Type.ATTRIBUTE)) {
            ...
            if (attr.getLookup().lookup(TagsMarker.class) != null) {
                return 1.0;
            }
        }
        return 0.0;
    ...
    public Presenter<SmallTextDisplay> createPresenter(
        ...
        if (canCreate(field) > 0.0) {
            RtcWorkItemAttribute<?> attr = field.getLookup().lookup(RtcWorkItemAttribute.class);
            return new TagsPresenter(display, attr.getLookup().lookup(TagsMarker.class).getAttribute(), wiwc);
        }
     ...
     * 
     */
    private static class TagsPresenter extends BasicPresenter<SmallTextDisplay> implements InputHandler<String> {
         ...
         * @param wiwc
         */
        TagsPresenter(SmallTextDisplay display,
        ...
        public void valueEntered(String value) {
            try {
                ...
                getDisplay().setInfoStatus(SmallTextDisplay.Status.OK, NbBundle.getMessage(TagsPresenter.class, "TagsPresenter.input.infoMessage"));
            } catch (IllegalArgumentException ex) {
                getDisplay().setInfoStatus(SmallTextDisplay.Status.ERROR,
                        NbBundle.getMessage(TagsPresenter.class, "TagsPresenter.input.errorMessage"));
            }
        }
    }
}
