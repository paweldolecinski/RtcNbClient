...
public class StringAttributeExpressionWidgetFactory extends AbstractAttributeExpressionWidgetFactory {

    @Override
    public boolean canCreate(RtcEditableAttributeExpression expression,RtcQuery query) {
        return expression.getQueryAttribute().getLookup().lookup(RtcQueryAttributeStringValueChecker.class) != null;
    }
    ...
    public Widget createDescription(RtcEditableAttributeExpression expression,RtcQuery query, Scene scene) {
        return new StringMultiValueEditorWidget(
                scene,
                expression,
                expression.getQueryAttribute().getLookup().lookup(RtcQueryAttributeStringValueChecker.class));
    }
