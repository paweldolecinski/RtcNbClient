...
public class DurationAttributeExpressionWidgetFactory extends AbstractAttributeExpressionWidgetFactory {

    @Override
    public boolean canCreate(RtcEditableAttributeExpression expression,RtcQuery query) {
        return expression.getQueryAttribute().getLookup().lookup(RtcQueryAttributeDurationValueChecker.class) != null;
    }
    ...
    public Widget createDescription(RtcEditableAttributeExpression expression,RtcQuery query, Scene scene) {
        return new StringMultiValueEditorWidget(
                scene,
                expression,
                expression.getQueryAttribute().getLookup().lookup(RtcQueryAttributeDurationValueChecker.class));
    }
