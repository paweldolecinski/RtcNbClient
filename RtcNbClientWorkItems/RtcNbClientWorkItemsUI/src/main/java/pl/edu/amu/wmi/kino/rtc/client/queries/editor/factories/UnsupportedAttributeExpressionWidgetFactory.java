...
import pl.edu.amu.wmi.kino.rtc.client.queries.model.attributes.values.RtcQueryAttributeEmptyNameProvider;
...
public class UnsupportedAttributeExpressionWidgetFactory extends AbstractAttributeExpressionWidgetFactory {

    @Override
    public boolean canCreate(RtcEditableAttributeExpression expression,RtcQuery query) {
        return expression.getQueryAttribute().getLookup().lookup(RtcQueryAttributeEmptyNameProvider.class) != null;
    }

    @Override
    public Widget createDescription(RtcEditableAttributeExpression expression,RtcQuery query, Scene scene) {
        RtcQueryAttributeEmptyNameProvider p = expression.getQueryAttribute().getLookup().lookup(RtcQueryAttributeEmptyNameProvider.class);
        return new LabelWidget(scene, p.getName());
    }
