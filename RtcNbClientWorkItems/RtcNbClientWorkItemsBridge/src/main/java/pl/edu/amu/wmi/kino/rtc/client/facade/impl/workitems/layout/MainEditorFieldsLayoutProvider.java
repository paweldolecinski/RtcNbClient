 ...
 */
public class MainEditorFieldsLayoutProvider implements WorkItemLayoutProvider {

    public WorkItemFieldsLayout createFieldsLayout(IWorkItem wi, ActiveProjectAreaImpl area, RtcWorkItemViewTarget target) {
        if (target.equals(RtcWorkItemViewTarget.MAIN)) {
                    ...
                    for (AbstractPresentationDescriptor s : sections) {
                        ...
                        if (descriptions != null) {
                            ...
                            for (AbstractPresentationDescriptor d : descriptions) {
                                ...
                                // IAttribute attribute = getAttributeForId(desc
                                // .getAttributeId());
                                // // if (attribute != null
                                // && wi.hasAttribute(attribute)) {
                                ...
                                try {
                                    ...
                                    // FIXME non-attribute presentations
                                    if (desc.getKind().equals("com.ibm.team.workitem.kind.internal.attachments")) {
                                        sec.add(WorkItemLayoutField.createField(
                                                WorkItemLayoutField.Type.NON_ATTRIBUTE,
                                                Lookups.singleton(new AttachmentsMarker())));
                                    } else {
                                        sec.add(WorkItemLayoutField.createField(
                                                WorkItemLayoutField.Type.NON_ATTRIBUTE,
                                                Lookup.EMPTY));
                                    }
//                                    System.out.println("Non-attribute: " + desc.getKind());
                                }
                                // }
