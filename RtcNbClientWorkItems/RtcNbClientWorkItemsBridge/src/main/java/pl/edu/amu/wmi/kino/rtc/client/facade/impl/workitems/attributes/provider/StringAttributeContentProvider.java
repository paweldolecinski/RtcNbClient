...
public class StringAttributeContentProvider implements AttributeContentProvider {
	...
	public Lookup createLookup(final IAttribute ia, ActiveProjectAreaImpl area) {
		...
		if (AttributeTypes.SMALL_STRING.contains(type)) {
			...
			ic.add(new SmallTextMarker() {
				...
				public RtcWorkItemAttribute<String> getAttribute() {
					...
					try {
						...
						// shouldn't happen
						RtcLogger.getLogger(StringAttributeContentProvider.class)
			...
			return lookup;
		} else if (AttributeTypes.MEDIUM_STRING.contains(type)
				|| AttributeTypes.LARGE_STRING.contains(type)) {
			InstanceContent ic = new InstanceContent();
			Lookup lookup = new AbstractLookup(ic);
			return lookup;
		} else {
