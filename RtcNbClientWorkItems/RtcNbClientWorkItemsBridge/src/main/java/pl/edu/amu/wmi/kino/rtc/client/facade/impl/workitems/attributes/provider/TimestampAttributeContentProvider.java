...
public class TimestampAttributeContentProvider implements
		AttributeContentProvider {

	private class DateMarkerImpl implements DateMarker {
		private final AttributesManager am;
		private final IAttribute ia;

		private DateMarkerImpl(AttributesManager am, IAttribute ia) {
			this.am = am;
			this.ia = ia;
		}

		@Override
		public RtcWorkItemAttribute<Date> getAttribute() {
			RtcWorkItemAttribute<Date> attributeById = null;
			try {
				attributeById = am.getAttributeById(ia.getIdentifier(),
						Date.class);
			} catch (UnsupportedAttributeIdException ex) {
				// shouldn't happen
				RtcLogger.getLogger(TimestampAttributeContentProvider.class)
	...
	/* (non-Javadoc)
	 * @see pl.edu.amu.wmi.kino.rtc.client.facade.impl.workitems.attributes.provider.WorkItemAttributeContentProvider#createLookup(com.ibm.team.workitem.common.model.IAttribute, pl.edu.amu.wmi.kino.rtc.client.impl.connections.ActiveProjectAreaImpl)
	 */
	@Override
	public Lookup createLookup(final IAttribute ia, ActiveProjectAreaImpl area) {
		if (AttributeTypes.TIMESTAMP.equals(ia.getAttributeType())) {
			InstanceContent ic = new InstanceContent();
			Lookup lookup = new AbstractLookup(ic);
			final AttributesManager am = area.getLookup().lookup(
					AttributesManager.class);
			ic.add(new DateMarkerImpl(am, ia));
			return lookup;
		} else {
			return null;
		}
