 ...
 */
public class KinoMultiViewCardLayoutCloneableTopComponent extends CloneableTopComponent {

    private static final long serialVersionUID = 456134567L;
    //kino multiView
    ...
    private HashMap<KinoMultiViewDescription, KinoMultiViewElement> elements =
            new HashMap<KinoMultiViewDescription, KinoMultiViewElement>();
    //toolbar panel
    private JComponent currentToolbar;
    private JPanel switchButtonsPanel;
    private HashMap<ButtonModel, KinoMultiViewDescription> buttonsToDesc =
            new HashMap<ButtonModel, KinoMultiViewDescription>();
    ...
    //component panel
    private HashMap<KinoMultiViewDescription, String> components =
            new HashMap<KinoMultiViewDescription, String>();
    private int uniqueComponentId = 0;
    //lookup
    private Lookup lookup;
    private Object lock = new Object();

    public KinoMultiViewCardLayoutCloneableTopComponent() {
    }

    public KinoMultiViewCardLayoutCloneableTopComponent(KinoMultiViewGroup[] groups, KinoMultiViewHeader header, KinoMultiViewCloseHandler closeHandler, KinoMultiViewTopComponentTabPosition position) {
        this.groups = groups;
        this.header = header;
        ...
        synchronized (lock) {
            descriptions =
                    (ArrayList<KinoMultiViewDescription>[]) new ArrayList[groups.length + 1];

            switchButtonsPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));

            int nonZeroCheck = 0;
            for (int i = 0; i < groups.length; i++) {
                descriptions[i] = new ArrayList<KinoMultiViewDescription>();
                descriptions[i].addAll(Arrays.asList(groups[i].getDescriptions()));
...
//        return buttonsToDesc.get(switchButtonGroup.getSelection());
    ...
    private KinoMultiViewElement getMVElement(KinoMultiViewDescription description) {
        ...
        synchronized (lock) {
            ...
            for (List<KinoMultiViewDescription> list : descriptions) {
                for (KinoMultiViewDescription d : list) {
                    ...
                    tmp.addItemListener(new KinoMultiViewSwitchButtonItemListener(d));
    ...
    private class KinoMultiViewGroupListenerImpl implements KinoMultiViewGroupListener {
        ...
        public void eventFired(KinoMultiViewGroupEvent event) {
            synchronized (lock) {
                ...
                descriptions[index].addAll(Arrays.asList(groups[index].getDescriptions()));
    ...
    private class KinoMultiViewLookupProviderImpl implements Provider {
    ...
    private class KinoMultiViewSwitchButtonItemListener implements ItemListener {
        ...
        public KinoMultiViewSwitchButtonItemListener(KinoMultiViewDescription desc) {
            this.desc = desc;
        }
