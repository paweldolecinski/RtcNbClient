 ...
 */
public class RtcReportReferenceChildrenImpl implements RtcReportReferenceChildren {

    private final IFolderDescriptor folder;
    private final ActiveProjectArea activeProjectArea;

    public RtcReportReferenceChildrenImpl(IFolderDescriptor folder, ActiveProjectArea activeProjectArea) {
        this.folder = folder;
    ...
    public RtcReportReference[] getChildren() {
        ...
        try {
            ...
            if (folder.isShared()) {
                List<IFolderDescriptor> root = reportManager.fetchFolderDescriptors(null, projectArea, null, folder, IReportManager.SHARED, null);
                for (IFolderDescriptor r : root) {
                    list.add(new RtcFoldertReference(r, activeProjectArea));
                }
                List<IReportQueryDescriptor> fetchReportDescriptors = reportManager.fetchQueryDescriptors(null, projectArea, null, null, folder, IReportManager.SHARED, null);
                for (IReportQueryDescriptor report : fetchReportDescriptors) {
                    list.add(new RtcReportReferenceImpl(report, folder));
                }
            } else {

                List<IFolderDescriptor> root = reportManager.fetchFolderDescriptors(repo.loggedInContributor(), projectArea, projectArea.getTeamAreas(), folder, IReportManager.PRIVATE, null);
                for (IFolderDescriptor r : root) {
                    list.add(new RtcFoldertReference(r, activeProjectArea));
                }
                List<IReportQueryDescriptor> fetchReportDescriptors = reportManager.fetchQueryDescriptors(repo.loggedInContributor(), projectArea, null, null, folder, IReportManager.PRIVATE, null);
                for (IReportQueryDescriptor report : fetchReportDescriptors) {
                    list.add(new RtcReportReferenceImpl(report, folder));
                }
            }
            ...
            RtcLogger.getLogger(RtcReportReferenceChildrenImpl.class).log(Level.SEVERE, ex.getLocalizedMessage(), ex);
        }
